/**
 * Migration 0003 — survey_responses table.
 *
 * Inlined as TS string constants so the bundled server.js has no runtime
 * file-path dependency (bundle-safe per SPEC §12.3 / D12).
 *
 * The survey_responses table is append-only (no UPDATE or DELETE except as
 * part of subject erasure per SPEC §6.5 WP-GDPR-SCAFFOLD).
 */
