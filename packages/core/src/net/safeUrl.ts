...
export interface SafeUrlOptions {
  /** Allow http:// (e.g. a local test server). Default false. */
  allowInsecure?: boolean
  /** Allow loopback / private / link-local hosts. Default false. */
 ...
 */
export function assertSafeBaseUrl(raw: string, opts: SafeUrlOptions = {}): URL {
  let url: URL
  try {
    url = new URL(raw)
  } catch {
 ...
 */
export function assertSameOrigin(candidate: string, baseOrigin: string): void {
  let url: URL
  try {
