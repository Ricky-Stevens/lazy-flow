 ...
 *
 * Tests are run against an in-memory NodeSqliteStore seeded from the
...
async function seedStore(store: NodeSqliteStore): Promise<void> {
  // Organisation
 ...
 */
function buildIssueRaw(issue: (typeof baseOrg.jiraIssues)[number]): string {
  const base = JSON.parse(issue.raw) as Record<string, unknown>
...
function mapIdentityToJiraAccountId(identityId: string | null): string | null {
...
function getDisplayName(identityId: string | null): string {
 ...
 */
async function embedChangelogInIssueRaw(store: NodeSqliteStore): Promise<void> {
  for (const [issueId, transitions] of Object.entries(baseOrg.issueTransitions)) {
    ...
    const raw = JSON.parse(issue.raw) as Record<string, unknown>
...
describe('stitchPersons — distinct-humans-same-name', () => {
  it('john.smith@acme.com and john.smith@vendor.com do NOT auto-merge', async () => {
    ...
    const db = new DatabaseSync(':memory:')
    migrate(db, 'up')
    const freshSt = new NodeSqliteStore(':memory:')
    freshSt.db.close()
    ;(freshSt as { db: DatabaseSync }).db = db
...
describe('identity audit regressions', () => {
  ...
  it('resolveIdentities enriches a github_login identity raw with numeric id + GitHub-verified email', async () => {
    ...
    const raw = JSON.parse(ident?.raw ?? '{}') as Record<string, unknown>
