...
import type { Identity, Person } from '../domain/types.js'
 ...
 */
function extractVerifiedEmail(raw: string): string | null {
  try {
 ...
 */
function primaryAccountRef(identity: Identity): string | null {
  if (identity.kind === 'github_login') {
    try {
      const parsed = JSON.parse(identity.raw) as Record<string, unknown>
...
export interface StitchPersonsOptions {
  /** ISO timestamp for new rows (default: now). */
  now?: string
  /** Per-org bot allowlist. */
  botAllowlist?: ReadonlyArray<string>
}

export interface StitchPersonsResult {
 ...
 */
export async function stitchPersons(
  store: Store,
  options: StitchPersonsOptions = {},
): Promise<StitchPersonsResult> {
  ...
  await store.transaction(async () => {
    ...
    // ------------------------------------------------------------------------
    const personIdOf = new Map<string, string | null>()
    ...
    const verifiedEmailToPersonId = new Map<string, string>()
    const jiraEmailToPersonId = new Map<string, string>()
    // local-part → commit_email identities (Tier 2c)
    const commitEmailsByLocalPart = new Map<string, Identity[]>()
    ...
    const tokenIndex = new Map<string, Identity[]>()
    ...
    for (const id of nonBots) {
      ...
      if (id.kind === 'jira_account' && pid !== null) {
        try {
          const jiraEmail = (
            JSON.parse(id.raw) as { emailAddress?: string }
...
function nameFromIdentity(identity: Identity): string | null {
  try {
    const parsed = JSON.parse(identity.raw) as Record<string, unknown>
    const name =
...
async function appendQueueEntry(
  store: Store,
  a: Identity,
  b: Identity,
