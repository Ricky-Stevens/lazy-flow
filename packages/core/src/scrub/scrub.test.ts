...
describe('scrubRawPayload', () => {
  ...
  it('scrubs free-text in string fields', () => {
    ...
    expect(out.body as string).toContain('[REDACTED_SECRET]')
    expect(out.body as string).toContain('[REDACTED_EMAIL]')
    expect(out.body as string).not.toContain('ghp_')
    expect(out.body as string).not.toContain('user@corp.com')
  })
