 ...
 *   3. Seed an in-memory NodeSqliteStore with batch snapshot rows and time a
...
import type { ThroughputInputs } from './flow/throughput.js'
import { throughput } from './flow/throughput.js'
import type { FlowIssueRecord, TransitionRecord } from './flow/types.js'
 ...
 */
function buildScaledIssues(n: number): FlowIssueRecord[] {
  const issues: FlowIssueRecord[] = []

  for (let clone = 0; clone < n; clone++) {
    for (const issue of baseOrg.jiraIssues) {
      // Collect transitions for this issue from the keyed map
      const rawTransitions: readonly (typeof baseOrg.issueTransitions)[string][number][] =
        baseOrg.issueTransitions[issue.id] ?? []

      const transitions: TransitionRecord[] = rawTransitions.map((t) => ({
        id: `${t.id}-c${clone}`,
...
describe('perf budgets (WP-PERF, SPEC §15)', () => {
  ...
  `snapshot read: 90-day window over ${CLONE_N * 5} snapshot rows < ${SNAPSHOT_READ_BUDGET_MS}ms`, async () => {
    ...
    const SCOPE_TYPE = 'team' as const
