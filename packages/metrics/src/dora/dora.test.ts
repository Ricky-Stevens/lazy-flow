 ...
 *
 * Uses the baseOrg dataset and seeds an in-memory NodeSqliteStore
...
import type { DeployIncidentLink, DeployRecord, IncidentRecord, PrRecord } from './types.js'
...
async function seedStore(): Promise<NodeSqliteStore> {
...
const deploys: DeployRecord[] = baseOrg.deployments.map((d) => ({
  id: d.id,
  repoId: d.repoId,
  sha: d.sha,
...
const deployIncidentLinks: DeployIncidentLink[] = baseOrg.deployIncidentLinks.map((l) => ({
  deployId: l.deployId,
  incidentIssueId: l.incidentIssueId,
}))

const prs: PrRecord[] = baseOrg.pullRequests.map((pr) => ({
  id: pr.id,
  repoId: pr.repoId,
...
function buildIncidentRecords(): IncidentRecord[] {
...
describe('deploymentFrequency', () => {
  ...
  it('filters by environment — staging deploys not counted for production', () => {
    const stagingDeploy: DeployRecord = {
      id: 'staging-1',
...
describe('changeFailureRate', () => {
  ...
  it('deploys with no linked incidents → rate 0', () => {
    const noLink: DeployIncidentLink[] = []
...
describe('recoveryTime', () => {
  ...
  it('reopened incident: MTTR anchored on FIRST Done (1h), not final Done (22h later)', () => {
    ...
    const firstResolvedMs = new Date(incident1.firstResolvedAt as string).getTime()
...
describe('DORA integration — seeded store', () => {
  it('runSync seeds store; deployment frequency computable from synced data', async () => {
    ...
    const inputs: DeployRecord[] = syncedDeploys.map((d) => ({
      id: d.id,
      repoId: d.repoId,
      sha: d.sha,
      environment: d.environment,
