...
import type { MetricScope } from '@lazy-flow/core'
import { describe, expect, it } from 'vitest'
import type { VisibilityFilterResult } from './index.js'
import {
...
type RowShape = { scopeType: MetricScope; scopeId: string; metric: string }

function makeRow(scopeType: MetricScope, scopeId: string, metric = 'test'): RowShape {
...
describe('policy matrix — snapshot-write + tool-read combined', () => {
  it('public: person snapshot persisted + person rows visible in tool-read', () => {
    expect(shouldPersistPersonSnapshot('person', 'public')).toBe(true)
    const result: VisibilityFilterResult<RowShape> = applyVisibilityFilter(
      [TEAM_ROW, PERSON_A],
      'public',
      null,
    )
