/**
 * Estimation Accuracy — Agile Group E (SPEC §8.5)
 *
 * Tie-corrected Spearman rank correlation between story points and actual cycle time.
 *
 * Rules (SPEC §8.5):
 *   - Tie-corrected Spearman (Fibonacci points are heavily tied at small n).
 *   - Minimum n guard: requires n ≥ 5 pairs (suppressed otherwise).
 *   - Significance guard: suppressed when not statistically significant.
 *   - Excludes reopened issues and 0-point issues.
 *
 * Statistical significance: for n ≥ 5, compute t = r√(n−2)/√(1−r²)
 * and compare against a two-tailed t critical value at α=0.05.
 * For small n, use a conservative lookup table.
 */
...
export interface EstimationPair {
  issueId: string
  storyPoints: number
  /** Cycle time in seconds (or hours — unit must be consistent across all pairs). */
  cycleTimeSeconds: number
  wasReopened: boolean
}

export interface EstimationAccuracyInputs {
  pairs: readonly EstimationPair[]
  /** Minimum n required (default 5). */
  minN?: number
  /** Significance level α (default 0.05). */
  alpha?: number
}

export interface EstimationAccuracyResult extends MetricResult {
  /** Tie-corrected Spearman ρ in [−1, 1], or null if suppressed. */
  readonly spearman: number | null
  readonly sampleSize: number
  readonly isSignificant: boolean | null
  /** True when result is suppressed due to small n or non-significance. */
 ...
 */
export function tiedSpearman(xs: readonly number[], ys: readonly number[]): number | null {
  const n = xs.length
  if (n < 2) return null
  if (xs.length !== ys.length) return null

  function tiedRanks(values: readonly number[]): number[] {
 ...
 */
const T_CRITICAL: Record<string, Record<number, number>> = {
...
const Z_CRITICAL: Record<string, number> = { '0.1': 1.645, '0.05': 1.96, '0.01': 2.576 }

function tableKey(alpha: number): string {
...
export function isSpearmanSignificant(rho: number, n: number, alpha = 0.05): boolean {
...
export const estimationAccuracy: MetricModule<EstimationAccuracyInputs, EstimationAccuracyResult> =
  {
    ...
    compute(inputs, asOf): EstimationAccuracyResult {
