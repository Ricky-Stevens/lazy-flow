...
import type { BlameRecord, FileComplexity } from '@lazy-flow/code-analysis'
...
import type { CodeChangeRecord, ComplexitySnapshot } from './types.js'
...
describe('halocAggregate', () => {
  ...
  it('aggregates across multiple changes', () => {
    ...
    expect(result.avgHalocPerChange as number).toBeCloseTo(result.totalHaloc / 2, 5)
  })
...
describe('reworkChurn', () => {
  ...
  const blameRecords: BlameRecord[] = [
...
describe('nagappanBall', () => {
  ...
  it('zero windowDays → M2 null (zero denominator)', () => {
    ...
    expect(Number.isNaN(result.m2ChurnRate as unknown as number)).toBe(false)
  })
})
...
describe('complexityDelta', () => {
  ...
  const baseComplexity: FileComplexity = {
    language: 'typescript',
    functions: [
      {
        name: 'widget',
  ...
  const headComplexity: FileComplexity = {
    language: 'typescript',
    functions: [
      {
        name: 'widget',
  ...
  const baseSnap: ComplexitySnapshot = { path: 'src/widget.ts', complexity: baseComplexity }
  const headSnap: ComplexitySnapshot = { path: 'src/widget.ts', complexity: headComplexity }
