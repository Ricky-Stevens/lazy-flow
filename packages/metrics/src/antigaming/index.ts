...
export type GamingFlag =
  ...
  | 'no_data'
  | 'insufficient_sample'
  | 'deploy_frequency_inflated'
  | 'cfr_suppressed'
  | 'lead_time_reset'
  | 'status_juggling'
  | 'trivial_pr_splitting'
...
export interface GamingDetectionResult {
  /** The detected flag, or 'ok' when no signal found. */
  flag: GamingFlag
  /** Human-readable explanation (empty string when flag === 'ok'). */
  reason: string
}
...
export interface DeployInflationInput {
  /** Environment label on the deployment. */
  environment: string
 ...
 */
export function detectDeployInflation(
  deploys: readonly DeployInflationInput[],
  opts: {
    /** The target environment (default: 'production'). */
    targetEnv?: string
    /** Gap in ms below which consecutive deploys to the same env are flagged as rapid. Default: 5 min. */
    rapidRedeployWindowMs?: number
  } = {},
...
export interface CfrSuppressionInput {
  deployId: string
  /** ISO-8601 deploy creation timestamp. */
  deployCreatedAt: string
  /**
   * ISO-8601 timestamp of a subsequent hotfix or revert event after this deploy,
   * or null if none.
   */
  hotfixOrRevertAt: string | null
  /** True when this deploy has at least one linked incident ticket. */
 ...
 */
export function detectCfrSuppression(
  deploys: readonly CfrSuppressionInput[],
): GamingDetectionResult {
...
export interface LeadTimeResetInput {
  prId: string
  /**
   * The authored_at of the merge/deploy commit — this is what git records
   * after a squash/rebase and is used as the lead-time anchor.
   */
  mergeCommitAuthoredAt: string
  /**
   * The authored_at of the first commit in the PR (before squash).
   * This is the honest work-start anchor.
   */
  firstCommitAuthoredAt: string
}
 ...
 */
export function detectLeadTimeReset(
  prs: readonly LeadTimeResetInput[],
...
export interface StatusJugglingInput {
  issueId: string
  /**
   * All transitions for this issue, sorted ascending by transitionedAt.
   * Each entry carries the fromStatusId and toStatusId.
   */
 ...
 */
export function detectStatusJuggling(
  issues: readonly StatusJugglingInput[],
  opts: {
    /** Time window in ms within which a round-trip is flagged. Default: 1 hour. */
    windowMs?: number
    /** Minimum round-trips within the window to trigger the flag. Default: 2. */
...
export interface TrivialPrSplittingInput {
  prId: string
  authorPersonId: string
  /** HALOC size of this PR (or additions+deletions if haloc unavailable). */
  size: number
  /** ISO-8601 PR creation timestamp. */
  createdAt: string
}
 ...
 */
export function detectTrivialPrSplitting(
  prs: readonly TrivialPrSplittingInput[],
  opts: {
    /** Window in ms to look for a burst of tiny PRs. Default: 24 hours. */
    windowMs?: number
    /** Minimum number of tiny PRs in the window to flag. Default: 5. */
    minPrs?: number
    /** Maximum HALOC for a PR to be considered "trivial". Default: 10. */
    maxSize?: number
  } = {},
): GamingDetectionResult {
 ...
 */
export const GOODHART_SENSITIVE_METRICS: ReadonlySet<string> = new Set([
...
export interface GoodhartWarning {
  metricId: string
  warning: string
}
 ...
 */
export function goodhartWarning(metricId: string): GoodhartWarning | null {
 ...
 */
export function assertNoCompositeProductivityNumber(calledFunctionName: string): void {
