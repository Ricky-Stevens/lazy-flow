...
const prs: PrInput[] = baseOrg.pullRequests.map((pr) => ({
  id: pr.id,
  repoId: pr.repoId,
...
const reviews: ReviewInput[] = baseOrg.reviews.map((r) => ({
  nodeId: r.nodeId,
  prId: r.prId,
...
const reviewComments: ReviewCommentInput[] = baseOrg.reviewComments.map((c) => ({
  nodeId: c.nodeId,
  prId: c.prId,
...
const deploys: DeployInput[] = baseOrg.deployments.map((d) => ({
  id: d.id,
  repoId: d.repoId,
  sha: d.sha,
...
describe('prCycleTime', () => {
  ...
  it('no merged PRs → value null, dataQuality no_data', () => {
    const openPrs: PrInput[] = prs.map((pr) => ({ ...pr, state: 'open', mergedAt: null }))
...
describe('timeToFirstReview', () => {
  ...
  it('no PRs with reviews → value null', () => {
    ...
    const noPrs: PrInput[] = [{ ...basePr, firstReviewAt: null, state: 'merged' }]
...
describe('prSize', () => {
  ...
  it('uses haloc when provided', () => {
    const halocPrs: PrInput[] = prs.map((pr) => ({
      ...pr,
...
describe('reviewerLoad — Gini', () => {
  ...
  it('giniCoefficient — all reviews by one person gives 1', () => {
    ...
    expect(g as number).toBeGreaterThan(0)
    expect(g as number).toBeLessThan(1)
  })
})
...
describe('commentsPerPr', () => {
  it('computes average comments per merged PR', () => {
    ...
    expect(result.averageComments as number).toBeCloseTo(1 / 3, 3)
  })
})
...
describe('ciHealth', () => {
  ...
  it('deterministic re-runs (identical conclusions) are NOT flaky', () => {
    ...
    const checkRuns: CheckRunInput[] = ['cr-a', 'cr-b', 'cr-c'].map((nodeId) => ({
      nodeId,
      repoId: IDS.repoAlpha,
      headSha: 'sha-stable',
      name: 'ci/test',
