/**
 * Jira workflow & scheme discovery (WP-JIRA-WORKFLOW).
 *
 * Discovers workflows and workflow schemes, resolves
 * issue → (project, issuetype) → workflow_id, and populates
 * `workflows` / `workflow_scheme_mappings` in the Store.
 *
 * This fixes the orphan FK from `flow_state_models.workflow_id` (SPEC §6.2).
 */

import type { Store, Workflow, WorkflowSchemeMapping } from '@lazy-flow/core'
 ...
 * if none found).
 */
export async function ingestWorkflows(
  store: Store,
  client: JiraClient,
  projectId: string,
  now: string,
  ...
  let targetScheme = rawSchemes.find(
    (s) =>
      ...
      (Array.isArray(s.projects) && s.projects.some((p: unknown) => String(p) === projectId)),
  )
  ...
  if (targetScheme) {
    ...
    if (mappings.length > 0) {
      for (const m of mappings) {
        ...
        const mapping: WorkflowSchemeMapping = {
          projectId,
      ...
      if (targetScheme.defaultWorkflow) {
        const defaultWfId = resolveWorkflowId(targetScheme.defaultWorkflow as string, workflowMap)
        if (defaultWfId) {
          const mapping: WorkflowSchemeMapping = {
            projectId,
 ...
 */
export async function ingestWorkflowsFromDataset(
  store: Store,
  workflows: Array<{ workflowId: string; name: string }>,
  mappings: Array<{ projectId: string; issueType: string; workflowId: string }>,
  now: string,
): Promise<WorkflowSyncResult> {
  let workflowsUpserted = 0
  let mappingsUpserted = 0
...
function resolveWorkflowId(nameOrId: string, workflowMap: Map<string, Workflow>): string | null {
  // Exact id match
