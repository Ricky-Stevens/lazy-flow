...
export interface BlameRecord {
  /** 1-based line number. */
  line: number
  /** When this line was last changed (Unix epoch ms or a Date). */
  lastChangedAt: Date | number
  /** The author who last changed this line. */
  author: string
}

/** Possible work-type buckets. */
export type WorkType = 'New' | 'Legacy-Refactor' | 'Help-Others' | 'Rework'

/** Per-line classification result. */
export interface LineWorkType {
  line: number
  workType: WorkType
}

/** Aggregate work-type result. */
export interface WorkTypeResult {
  /** Total lines classified. */
  total: number
  /** Counts per bucket. */
  counts: Record<WorkType, number>
  /** Rework as a percentage of total (null when total=0). */
...
/** Options for {@link classifyWorkType}. */
export interface WorkTypeOptions {
  /**
   * Author whose changes we are classifying (e.g. a commit author string).
   * Must match `BlameRecord.author` exactly.
   */
  author: string
  /**
   * Blame records for the changed lines.
   * A line not present in `blameRecords` is treated as a new line.
   */
  blameRecords: BlameRecord[]
  /**
   * Reference "now" timestamp (Date or Unix epoch ms).
   * **Must be injected** — never call Date.now() here.
   */
  now: Date | number
  /**
   * Lines to classify.  Typically the added/modified line numbers from the diff.
   * If omitted, all lines in `blameRecords` are classified.
   */
  lines?: number[]
  /**
   * Age window in days.  Lines last changed within this window are considered
   * "recent".  Defaults to 30 (Flow parity, SPEC §8.6 / D7).
   */
  windowDays?: number
}
 ...
 */
export function classifyWorkType(options: WorkTypeOptions): WorkTypeResult {
  const { author, blameRecords, now, windowDays = DEFAULT_WINDOW_DAYS } = options
  const nowMs = toMs(now)
  const windowMs = windowDays * 24 * 60 * 60 * 1000

  // Build a lookup by line number
  const blameByLine = new Map<number, BlameRecord>()
