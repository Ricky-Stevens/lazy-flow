 ...
 *  cwd-relative.  Grammar files are loaded as Buffer via readFileSync so
...
// biome-ignore lint/suspicious/noExplicitAny: web-tree-sitter@0.20.x ships CJS with no named ESM exports
const Parser: any = _require('web-tree-sitter') // eslint-disable-line @typescript-eslint/no-explicit-any

// ── WASM paths (bundle-safe) ─────────────────────────────────────────────────

/**
 * Optional grammar directory override — used by the bundled MCP server to
 * redirect grammar resolution to server/dist/grammars/ (next to server.js)
...
function grammarPath(name: string): string {
  if (grammarDirOverride !== null) {
    // Bundle path: grammars sit next to the server.js in server/dist/grammars/
...
const GRAMMAR_NAMES: Record<SupportedLanguage, string> = {
...
// biome-ignore lint/suspicious/noExplicitAny: Parser instance
let parserInstance: any = null
// biome-ignore lint/suspicious/noExplicitAny: Language from CJS module
...
// biome-ignore lint/suspicious/noExplicitAny: Language from CJS module
async function getLanguage(lang: SupportedLanguage): Promise<any> {
  const cached = languageCache.get(lang)
  if (cached) return cached
  const wasm = readFileSync(grammarPath(GRAMMAR_NAMES[lang]))
  // biome-ignore lint/suspicious/noExplicitAny: Parser.Language is on the namespace (0.20.x)
  const language = await (Parser.Language as any).load(wasm)
  languageCache.set(lang, language)
  return language
}

// biome-ignore lint/suspicious/noExplicitAny: Parser instance type
...
export interface FunctionComplexity {
  /** Function name (anonymous → '<anonymous>'). */
  name: string
  /** 1-based start line. */
  startLine: number
  /** 1-based end line. */
...
/** File-level complexity result. */
export interface FileComplexity {
  language: SupportedLanguage
  functions: FunctionComplexity[]
  /** Sum of cyclomatic over all functions. */
  totalCyclomatic: number
  /** Sum of cognitive over all functions. */
  totalCognitive: number
}

/** Delta between head and base for one function (matched by name). */
export interface ComplexityDelta {
  name: string
  startLine: number
  /** null = function is new in head (no base). */
...
function isBoolChainNode(node: SNode): boolean {
...
function getFunctionBody(funcNode: SNode): SNode | null {
...
function getFunctionName(node: SNode): string {
...
function isRecursiveCall(callNode: SNode, funcName: string): boolean {
 ...
 */
function countChainRuns(chainRoot: SNode): number {
  const ops: string[] = []

  function collect(node: SNode): void {
...
function computeCyclomatic(funcNode: SNode): number {
  let score = 1
...
function computeCognitive(funcNode: SNode, funcName: string): number {
  let score = 0
   ...
   */
  function walk(node: SNode, depth: number, insideElseClause = false): void {
    const t = node.type
...
function extractFunctions(root: SNode): Array<{ node: SNode; name: string; topLevel: boolean }> {
  const results: Array<{ node: SNode; name: string; topLevel: boolean }> = []

  function walk(node: SNode, insideFunction: boolean): void {
...
export async function analyzeComplexity(
  ...
  const allFunctions = extractFunctions(tree.rootNode as SNode)
  const functions: FunctionComplexity[] = allFunctions.map(({ node, name }) => ({
    name,
 ...
 */
export async function computeComplexityDelta(
  baseSource: string,
  headSource: string,
  language: SupportedLanguage,
): Promise<FileDelta> {
  ...
  const baseByName = new Map<string, FunctionComplexity[]>()
  ...
  const deltas: ComplexityDelta[] = head.functions.map((hf) => {
    const candidates = baseByName.get(hf.name)
    let bf: FunctionComplexity | undefined
    if (candidates) {
