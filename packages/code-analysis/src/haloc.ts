...
interface ParsedFile {
  ...
  hunks: ParsedHunk[]
  isBinary: boolean
  /** rename target (non-null when diff header says "rename … to …") */
  renameTo: string | null
}

/** Options for {@link computeHaloc}. */
export interface HalocOptions {
  /**
   * Extra glob patterns for paths considered generated/vendored.
   * Matched against the file's path component (forward-slash normalised).
   * These are appended to the built-in defaults.
   */
  additionalGeneratedGlobs?: string[]
  /**
   * When false (default), whitespace-only lines in hunks are treated as real
   * changes.  When true, lines that consist only of `+`/`-` followed by
   * optional whitespace do NOT count toward insertions/deletions (mirrors
   * `git diff -w`).
   */
  whitespaceSensitive?: boolean
}

/** Per-file result broken down by classification. */
export interface FileHalocResult {
  path: string
  haloc: number
  insertions: number
  deletions: number
...
/** Aggregate result from {@link computeHaloc}. */
export interface HalocResult {
  /** Total HALOC from non-binary, non-generated files. */
  haloc: number
  /** HALOC from binary file hunks (never zero'd; surfaced separately). */
  binaryHaloc: number
  /** HALOC from generated/vendored files (excluded from `haloc`). */
  generatedHaloc: number
  /** Per-file breakdown. */
  files: FileHalocResult[]
}

// ── Default generated/vendored glob patterns ─────────────────────────────────

const DEFAULT_GENERATED_GLOBS: readonly string[] = [
  '*-lock.json',
  'package-lock.json',
  'yarn.lock',
  'pnpm-lock.yaml',
...
function isGeneratedPath(path: string, extraGlobs: readonly string[]): boolean {
 ...
 */
function parseDiff(diff: string, wsInsensitive: boolean): ParsedFile[] {
 ...
 */
export function computeHaloc(diff: string, options: HalocOptions = {}): HalocResult {
