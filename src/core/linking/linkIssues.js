 ...
 */
export async function linkIssues(store, options = {}) {
    ...
    for (const org of orgs) {
      ...
      for (const repo of repos) {
        ...
        for (const pr of prs) {
          ...
          function addCandidate(key, source, confidence) {
 ...
 */
export async function linkageRate(store) {
  const orgs = await store.listOrganisations()
  let mergedTotal = 0
  let linked = 0
