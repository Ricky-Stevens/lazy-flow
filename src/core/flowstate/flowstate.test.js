 ...
 * fallback).
 *
 * All tests run against an in-memory BunSqliteStore seeded from the
 ...
 */
async function seedBaseOrg(store) {
 ...
 */
async function ensureWorkflow(store, workflowId) {
...
describe('seedFlowStateModel — out-of-order validFrom', () => {
  const statuses = [{ id: 's-ooo', name: 'In Progress', category: 'indeterminate' }]
...
describe('ensureFallbackMapping', () => {
  it('inserts low-confidence rows for a workflow that has never been seeded', async () => {
    ...
    const statuses = [
      { id: 'status-fb-1', name: 'To Do', category: 'new' },
      { id: 'status-fb-2', name: 'In Progress', category: 'indeterminate' },
      { id: 'status-fb-3', name: 'Done', category: 'done' },
    ]
