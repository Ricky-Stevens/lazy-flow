...
import { BunSqliteStore, linkageRate, linkIssues, migrate } from '../core/index.js'
...
// ---------------------------------------------------------------------------
// runSync — engine-version-bump rederive hook (SPEC §8.6)
// ---------------------------------------------------------------------------

describe('runSync — rederive hook', () => {
  it('invokes the injected rederive hook with the sync timestamp and surfaces its summary', async () => {
    const store = makeStore()
    ...
    const result = await runSync(
      ...
      'backfill',
      {
        ...
        rederive: async (now) => {
          calledWith = now
          return { bumpDetected: true, markedStale: 3, recomputed: 3 }
        },
      },
    )

    expect(calledWith).toBe(NOW)
    expect(result.rederive).toEqual({ bumpDetected: true, markedStale: 3, recomputed: 3 })
    expect(result.errors).toHaveLength(0)
  })

  it('returns rederive=null when no hook is supplied', async () => {
    const store = makeStore()
    ...
    expect(result.rederive).toBeNull()
  })

  it('records a hook failure in errors without failing the sync', async () => {
    const store = makeStore()
    ...
    expect(result.rederive).toBeNull()
    expect(result.errors.some((e) => e.startsWith('rederive: boom'))).toBe(true)
    // The sync itself still succeeded.
    expect(result.github.repos.length).toBeGreaterThan(0)
  })
})
