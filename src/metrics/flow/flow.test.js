...
const BOARD_COLUMNS = baseOrg.boardColumns.map((col) => ({
...
function buildFlowStateResolver(overrides = {}) {
  const map = {
  ...
  return (statusId, _at) => map[statusId] ?? null
}

// Build FlowIssueRecord from baseOrg jira issue + transitions
function buildFlowIssue(issueId) {
...
describe('cycleTime', () => {
  ...
  it('cycle time using a started-only board (where Selected IS started) is longer', () => {
    const boardWithSelectedAsStarted = [
...
describe('flowEfficiency', () => {
  it('per-issue estimator is NOT inflated by one zombie ticket (pooled vs per-issue)', () => {
    ...
    expect(result.p50).toBeGreaterThan(0.1)
