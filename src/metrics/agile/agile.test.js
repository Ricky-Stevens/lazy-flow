...
const membershipEvents = baseOrg.sprintMembershipEvents.map((e) => ({
  sprintId: e.sprintId,
  issueId: e.issueId,
  change: e.change,
...
const issues = baseOrg.jiraIssues.map((issue) => {
...
describe('sprintVelocity', () => {
  ...
  it('unmapped story-point field → velocity null (flagged, not 0)', () => {
    const unmappedIssues = issues.map((i) => ({
      ...i,
  ...
  it('kanban board → velocity null (degrades gracefully)', () => {
    const kanbanSprint = { ...sprint, type: 'kanban' }
...
describe('estimationAccuracy', () => {
  it('computes Spearman ρ for correlated estimates', () => {
    ...
    expect(result.spearman).toBeGreaterThan(0.9)
