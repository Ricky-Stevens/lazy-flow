...
function issueToJiraShape(issue) {
  const fields = {
...
function changelogHandlers() {
  return [
    http.get('*/rest/api/3/issue/:issueId/changelog', ({ request, params }) => {
      ...
      const transitions = issueId !== undefined ? (baseOrg.issueTransitions[issueId] ?? []) : []
