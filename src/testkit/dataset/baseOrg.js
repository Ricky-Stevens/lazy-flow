...
export const IDS = {
  ...
  // Check runs (CI status keyed by PR head sha)
  checkRun1Build: 'check-run-1-build', // pr-1 head, success
  checkRun1Test: 'check-run-1-test', // pr-1 head, failure
  checkRun4Build: 'check-run-4-build', // pr-4 head, success
...
// ---------------------------------------------------------------------------
// Commit detail (GET /repos/{o}/{r}/commits/{sha}) — stats + per-file patches
// ---------------------------------------------------------------------------

/**
 * The per-commit DETAIL payload. The commit LIST endpoint carries no stats, so
 * the ingester fetches this to populate real additions/deletions/HALOC. `haloc`
 * is the expected HALOC the ingester recomputes from the file patches.
 */

const commitDetails = Object.freeze([
  // commitA1 (alice, alpha): one file, 6 ins / 1 del → HALOC 6.
  Object.freeze({
    sha: IDS.commitA1,
    additions: 6,
    deletions: 1,
    ...
    files: Object.freeze([
      Object.freeze({
        ...
        patch:
          ...
          '+export function build(w: Widget) {\n' +
          '+  return w\n' +
          '+}\n' +
...
const pullRequests = Object.freeze([
  ...
  Object.freeze({
    ...
    deletedAt: null,
    raw: '{"number":1,"state":"closed","merged":true,"title":"fix: resolve ACME-2 widget issue","body":"Fixes ACME-2 — adds null check for widget","head":{"ref":"feat/widget-service","sha":"aaaa0000000000000000000000000001"},"base":{"ref":"main"}}',
    updatedAt: '2024-03-02T10:00:00Z',
  }),
  ...
  Object.freeze({
    ...
    deletedAt: null,
    raw: '{"number":2,"state":"closed","merged":true,"title":"fix: handle null widget","body":"No issue reference","head":{"ref":"fix/null-widget","sha":"aaaa0000000000000000000000000002"},"base":{"ref":"main"}}',
  ...
  Object.freeze({
    ...
    mergedByIdentityId: IDS.identityBobGh,
    deletedAt: null,
    raw: '{"number":4,"state":"closed","merged":true,"title":"feat: gadget endpoint for OLD-99","body":"Implements gadget endpoint. Resolves OLD-99 (was ACME-2 before project rename).","head":{"ref":"feat/gadget-endpoint","sha":"bbbb0000000000000000000000000001"},"base":{"ref":"main"},"merge_commit_sha":"bbbb0000000000000000000000000001"}',
    updatedAt: '2024-04-01T11:00:00Z',
  }),
])

// ---------------------------------------------------------------------------
// PR files (per-file diffs from GET /pulls/{n}/files)
// ---------------------------------------------------------------------------

/**
 * A changed file within a PR. `patch` is a real unified-diff body (hunk headers
 * + +/- lines), WITHOUT the `diff --git` header — exactly the shape GitHub's
 * files API returns. `haloc` is the expected HALOC the ingester recomputes from
 * the patch (Σ_hunk max(insertions, deletions)), surfaced here so metric tests
 * can assert a concrete value.
 */

const prFiles = Object.freeze([
  // pr-1 (alpha): two files. widget.ts = one hunk, 5 ins / 2 del → HALOC 5.
  Object.freeze({
    prId: IDS.pr1,
    repoId: IDS.repoAlpha,
    path: 'src/widget.ts',
    additions: 5,
    deletions: 2,
    haloc: 5, // max(5,2) in a single hunk
    status: 'modified',
    patch:
      '@@ -1,4 +1,7 @@\n' +
      ...
      '+export function build(w: Widget) {\n' +
      '+  if (w == null) {\n' +
      '+    throw new Error("null widget")\n' +
      '+  }\n' +
      '+  return w\n' +
      ' }',
  }),
  // pr-1 widget.test.ts = one hunk, 3 ins / 0 del → HALOC 3.
  Object.freeze({
    prId: IDS.pr1,
    repoId: IDS.repoAlpha,
    ...
    patch:
      ...
      '+test("rejects null", () => { expect(() => build(null)).toThrow() })\n' +
      '+test("returns widget", () => { expect(build({})).toEqual({}) })',
  }),
  // pr-4 (beta): one file. gadget.go = one hunk, 10 ins / 4 del → HALOC 10.
  Object.freeze({
    prId: IDS.pr4,
    repoId: IDS.repoBeta,
    path: 'internal/gadget/gadget.go',
    additions: 11,
    deletions: 4,
    haloc: 11, // max(11,4) in a single hunk
    status: 'modified',
    patch:
      '@@ -1,6 +1,12 @@\n' +
      ' package gadget\n' +
      ...
      '-// old\n' +
      '+import "errors"\n' +
      '+func Endpoint(id string) (string, error) {\n' +
      '+\tif id == "" {\n' +
...
const reviewComments = Object.freeze([
...
// ---------------------------------------------------------------------------
// Check runs (CI status, GET /commits/{ref}/check-runs)
// ---------------------------------------------------------------------------

/**
 * A CI check run for a PR head sha. The MSW check-runs handler keys these by
 * `headSha` (the ref the runs are fetched under), so a PR's head sha drives
 * which runs the ingester persists. Feeds pr.ci_health.
 */

const checkRuns = Object.freeze([
  // pr-1 head (commitA1): build passes, test fails → 1/2 pass rate.
  Object.freeze({
    nodeId: IDS.checkRun1Build,
    repoId: IDS.repoAlpha,
    headSha: IDS.commitA1,
    name: 'build',
    status: 'completed',
    conclusion: 'success',
    startedAt: '2024-03-01T08:10:00Z',
    completedAt: '2024-03-01T08:20:00Z',
    raw: '{"id":"check-run-1-build","node_id":"check-run-1-build","name":"build","status":"completed","conclusion":"success","started_at":"2024-03-01T08:10:00Z","completed_at":"2024-03-01T08:20:00Z"}',
    updatedAt: '2024-03-01T08:20:00Z',
  }),
  Object.freeze({
    nodeId: IDS.checkRun1Test,
    repoId: IDS.repoAlpha,
    headSha: IDS.commitA1,
    name: 'test',
    status: 'completed',
    conclusion: 'failure',
    startedAt: '2024-03-01T08:10:00Z',
    completedAt: '2024-03-01T08:25:00Z',
    raw: '{"id":"check-run-1-test","node_id":"check-run-1-test","name":"test","status":"completed","conclusion":"failure","started_at":"2024-03-01T08:10:00Z","completed_at":"2024-03-01T08:25:00Z"}',
    updatedAt: '2024-03-01T08:25:00Z',
  }),
  // pr-4 head (commitB1, beta-service): build passes.
  Object.freeze({
    nodeId: IDS.checkRun4Build,
    repoId: IDS.repoBeta,
    headSha: IDS.commitB1,
    name: 'build',
    status: 'completed',
    conclusion: 'success',
    startedAt: '2024-04-01T09:05:00Z',
    completedAt: '2024-04-01T09:15:00Z',
    raw: '{"id":"check-run-4-build","node_id":"check-run-4-build","name":"build","status":"completed","conclusion":"success","started_at":"2024-04-01T09:05:00Z","completed_at":"2024-04-01T09:15:00Z"}',
...
const jiraStatuses = Object.freeze([
  Object.freeze({ id: IDS.statusBacklog, name: 'Backlog', category: 'new' }),
  ...
  Object.freeze({ id: IDS.statusDone, name: 'Done', category: 'done' }),
])
...
const jiraProject = Object.freeze({
 ...
 */
const storyTransitions = Object.freeze([
 ...
 */
const incident1Transitions = Object.freeze([
  // Backlog → In Progress
  Object.freeze({
    id: 'tr-incident-01',
...
const incident2Transitions = Object.freeze([
  Object.freeze({
    id: 'tr-incident2-01',
...
const subtaskTransitions = Object.freeze([
  Object.freeze({
    id: 'tr-subtask-01',
...
const issueTransitions = Object.freeze({
 ...
 */
const sprintMembershipEvents = Object.freeze([
...
const boardConfigs = Object.freeze([Object.freeze({ boardId: IDS.boardId, type: 'scrum' })])
...
const workflowSchemeMappings = Object.freeze([
  Object.freeze({
    projectId: IDS.jiraProjectId,
    issueType: 'Story',
    workflowId: IDS.workflowId,
...
const prIssueLinks = Object.freeze([
  Object.freeze({
    prId: IDS.pr1,
    issueId: IDS.issueStory1,
    linkSource: 'regex',
...
const deployIncidentLinks = Object.freeze([
  Object.freeze({
 ...
 */
const historicalIssueKeys = Object.freeze([
 ...
 */
export const baseOrg = Object.freeze({
  org,
  repositories,
  persons,
  identities,
