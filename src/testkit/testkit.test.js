...
describe('mockGitHub', () => {
  ...
  it('GraphQL: PullRequestGraph returns pr-1 with paginated reviews', async () => {
    ...
    const pr1 = page1?.repository?.pullRequest
    ...
    const cursor1 = reviewsPage1?.pageInfo?.endCursor
    ...
    const pr1p2 = page2?.repository?.pullRequest
...
describe('withMockServer', () => {
  // withMockServer is called synchronously in the describe body so that its
  // internal beforeAll/afterEach/afterAll lifecycle hooks are registered during
  // test collection — bun:test (like vitest) requires hooks to be registered at
  // describe scope, not from inside another running hook. The returned server's
  // listen/close lifecycle is then driven by those registered hooks.
...
async function fetchGraphQLRaw(operationName, variables) {
